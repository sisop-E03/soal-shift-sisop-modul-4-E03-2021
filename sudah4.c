...
static struct fuse_operations xmp_oper = {
    ...
    .read = xmp_read,//
    .mkdir = xmp_mkdir,//
    .rmdir = xmp_rmdir,//masukwarning
    .mknod = xmp_mknod,//masuk info
    .rename = xmp_rename,//
    .write = xmp_write//MASUK INFO
    };

//no 4 yess
char *LOG="/home/vika/fuse.log";

//untuk level warning rmdir dan unlink
void LevelWarning(char* desc, char* path) {
	...
	fprintf(file_log, "WARNING::%d%d%d-%02d:%02d:%02d::%s::%s\n", 
		hari, bulan, tahun, jam, menit, detik, desc, path);
	fclose(file_log);
}

//untuk level info selain yang di warning
void LevelInfo(char* desc, char* path) {
	FILE *file_log = fopen(LOG, "a");
	...
	fprintf(file_log, "INFO::%d%d%d-%02d:%02d:%02d::%s::%s\n", 
		hari, bulan, tahun, jam, menit, detik, desc, path);
	fclose(file_log);
}
